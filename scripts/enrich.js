// OptimizeUp Extension v18.0.7 — Job Enrichment Injectable
// v18.0.7: new Upwork (Nuxt) detail UI — client stats from [data-test="about-client-container"];
//   rating "Rating is X out of 5", reviews "of N reviews", country/city from location-before-localtime.
//   (extractDescription already prefers [data-test="Description"].)
...
(function () {
  ...
  function extractClientStats() {
    ...
    // Country — legacy sidebar selectors first
    ...
    // v18.0.7: current Nuxt UI — client block is [data-test="about-client-container"];
    // location sits right before the client's local time, e.g. "USA Lake Mary 3:56 PM".
    if (!stats.country) {
      const ac = document.querySelector('[data-test="about-client-container"]');
      const m = ac && (ac.textContent || '').match(/([A-Z][A-Za-z.\- ]{1,40}?)\s+\d{1,2}:\d{2}\s*(?:AM|PM)/);
      if (m) {
        const loc = m[1].trim();
        const parts = loc.split(' ');
        stats.country = parts[0];
        if (parts.length > 1) stats.city = parts.slice(1).join(' ');
      }
    }
    ...
    // v18.0.7: rating now rendered as text "Rating is 4.9 out of 5" (old .air3-rating-value gone)
    if (stats.rating == null) {
      const rm = allText.match(/Rating is\s+([\d.]+)\s+out of 5/i);
      if (rm) { const n = parseFloat(rm[1]); if (!isNaN(n) && n >= 1 && n <= 5) stats.rating = n; }
    }
    ...
    // v18.0.7: current UI shows "4.85 of 13 reviews" (no parentheses); keep paren form as fallback
    const reviews = allText.match(/of\s+(\d+)\s+reviews?/i) || allText.match(/\(\s*(\d+)\s*reviews?\s*\)/i);
